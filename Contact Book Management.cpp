    ...
    //constructor to intialise the values.
		...
		void DeleteContactBySearch()
    	{
	    	...
	    	if(command==1)
	    	{
	        	...
	        	if(Dcheck==true)
	        	{
			    	...
			    	else if((command==1)&(temp->next==NULL))//if contact is the last node
			    	{
			    		temp->prev->next=NULL;
			    		delete temp;
			    		cout<<"  Contact Deleted Success Fully"<<endl;
				...
				{
					cout<<"  Contact of This Name Not Found."<<endl;//if there is no contact by this name.
			    }  
	    	}
	    	else if(command==2)
	    	{
	    	...
	    	{
	    		cout<<"  You Enter wrong Command"<<endl;//if there is no contact by this phone number
			}
	    }
        ...
        void EditContacts()
    	{
	    	...
	    	if(Ecommand==1)
	    	{
	    		...
	    		cin>>x;
	    		while(temp!=NULL)// Iterate through the contacts to find the one with the specified name.
	    		{
	    	    	if(temp->name==x)
		        	{
		        		cout<<"***********"<<endl;
	    	    ...
	    	    {
	    	    	cout<<"  Contact Not Found"<<endl;//There is no contact by this name.
				}
	        }
	    	else if(Ecommand==2)
	    	{
		...
		void Structure()
    	{
	    	...
	    	try
	    	{
				...
				else // If the entered command is out of range, throw an exception.
				{
					throw(Scommand);
				}
			}
			catch(int Scommand)
			{
				...
				Structure();// Handle the exception by presenting the menu again.
			}	
	    }
